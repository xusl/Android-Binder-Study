...
\begin{tabular}{|p{0.43\textwidth}|p{0.52\textwidth}|}\hline
    ...
    BC_REGISTER_LOOPER 时用到。\ref{BinderThreadPool}\\\hline
...
\begin{tabular}{|p{0.3\textwidth}|p{0.7\textwidth}|}\hline
    BC_TRANSACTION & 用于客户端向服务端发送请求数据\ref{BinderTransaction}\\
    BC_REPLY & 用于服务端向客户端发送回复数据。\\\hline
    BC_FREE_BUFFER & 释放一块映射的内存。\ref{BinderMemoryMap} \\\hline
    BC_INCREFS\newline BC_ACQUIRE\newline BC_RELEASE\newline BC_DECREFS &
    ...
    \ref{BinderThreadPool}。BC_REGISTER_LOOPER
...
远程 Binder 的实例化在 ProcessState::getStrongProxyForHandle\label{code:getStrongProxyForHandle}、
...
ProcessState::getWeakProxyForHandle 等等来实例化本地binder。
...
getService 后获得一个远程的binder， 再用这个binder 调用 接口 asInterface。
如 \ref{code:asInterface} 所述，asInterface 是本地接口（如 BpEneaBuffer）唯一
的实例化地方。 好像在 addService 时 ，service manager 创建了一个 BpBidner？ 
...
\begin{description*}
    ...
    \item [binder/handle]
        ...
        desc。
        传递本地Binder时使用binder域，指向Binder在服务进程中的地址。
    ...
    \item [flags]
            ...
            7\~{}0 bits 设置 binder_node 线程优先级，远程进程的对 \binder 的操作请求会由\binder
...
复并唤醒它。
因为可能会有许多客户端同时发起请求，服务进程为了提高效率往往开辟线程池并发处理收到的请求。
...
工作队列分进程和线程，分别记录在binder_proc\ref{BinderProcTodo}和
binder_thread\ref{BinderThreadTodo}的todo列表。等待队列也分进程和线程，
binder_proc wait\ref{BinderProcWait}队列以及binder_thread wait\ref{BinderThreadWait}队列 
...
\begin{itemize}
    \item 规则1：客户端发给服务端的请求数据包都提交到服务端的进程工作队列。除非
        \ref{ThreadStartOptimize}中的情况，来自T1的请求不是提交给P2的全局to-do队列，
        而是送入了T2的私有to-do队列。
...
实现了mmap()系统调用\ref{Bindermmap}，这对字符设备是比较特殊的，因为mmap()通常用在有物理存储介
...
binder_transaction_data结构本身以及\ref{proto:binderreturn}中所有消息的
